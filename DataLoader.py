...
from qgis.core import QgsVectorLayer, QgsFeature, QgsField, QgsGeometry, \
    QgsPointXY, QgsProject, QgsMapLayerProxyModel, \
...
class DataLoader:
    ...
    def run(self):
        ...
        # show the dialog
        if self.dlg.cbMapLayer.currentLayer() is None:
    ...
    def lst2arr(self, alist):
        """ Makes a comma separated string. If postgres is used, the string
        is formatted to be inserted as a native array"""
        ret = ','.join(alist)
        if self.database == "PG":
            ret = '{' + ret + '}'
        return ret
    ...
    def selectfile(self):
        ...
        To write a parser for another file format: The parser should read the
        file or files to be imported, then for each measurement populate an
        array with the following data:

        altitude, floating point number (above sea level, e.g. gps altitude)-
        sampling time, string (any format, but it should be consistent)
        height above ground1, floating point number, e.g. radar or laster alt
        height above ground2, floating point number, e.g. radar or laster alt
        ...
        spectre for detector 1, string: comma separated list of integers
        spectre for detector 2, string: comma separated list of integers
        timestamp: timestamp, string

        The values can all be strings in the call to insertpoints, but they
        need to be parseable as the above mentioned data types.

        Unknown data kan be represented by an empty string or None. These
        will both be stored as NULL.

        For each datapoint, call self.insertpoint(lat,lon, array,[filename])

        lat and lon is assumed to be be in WGS84. The point will be reprojected
        to whatever CRS is used in the layer it is being stored to

        The filename is by default the selected filename. For data types where
        each spectre is stored in a separate file, the filename for each file
        can be given. The function should set self.read to the number of lines
        that have been read successfully and self.readfailure to the number of
        lines that have had problems

        The reader function can throw an execption that will stop the reading
        and will print to the python console. Any data that have been read
        successfully befor the expection will stay stored.
        """
        ...
        toCRS = self.vl.crs()
        # The CRS of the vl is set to the same as the project
        # when the vl is created
        self.transformation = QgsCoordinateTransform(fromCRS, toCRS,
                                                     QgsProject.instance())
        try:
            self.read = 0
            self.readfailure = 0
            ...
            # https://gis.stackexchange.com/questions/110524/qgis-processing-progress  # noqa e501
            # To import other type of files, define some logic to recognice
            # them here and write a reader function that calls
            # self.insertpoints as defined above
            ...
            # Refreshes canvas and clears dialog to make it clear that the data
            # have been imported
            self.iface.mapCanvas().refreshAllLayers()
            self.dlg.leMission.clear()
            self.dlg.FileWidget.setFilePath("")
            if self.readfailure == 0:
                message = f"{self.read} points from file imported sucessfully to '{self.vl.name()}'"  # noqa e501
                level = Qgis.Success
            else:
                message = f"Problem reading {self.readfailure} points - ({self.read} read successfully)"  # noqa e501
                level = Qgis.Warning
            self.iface.messageBar().pushMessage("Data Loader", message,
                                                level=level)
        except unknownFileType as e:
            message = f"Unknown file type '{self.filename}'"
            print(e)
            self.iface.messageBar().pushMessage("Data Loader", message,
                                                level=Qgis.Critical)
        except Exception as e:
            print(e)
            message = f"Problem when importing '{self.filename}'"
            self.iface.messageBar().pushMessage("Data Loader", message,
                                                level=Qgis.Critical)
            raise e
    ...
    def readspe(self, filename):
        ...
        for filename in spefiles:
            try:
                ...
                self.insertpoint(float(gpsdata['Lat']), float(gpsdata['Lon']),
                                 insdata, directory + '/' + filename)
                # print(f'{filename} OK')
            except Exception as e:
                print(e)
    ...
    def parsespefile(self, filename, directory):
        ...
        insdata = [float(gpsdata['Alt']), date, 2, 2, None, float(temperature),
                   None, date, 1, None, dose, None, None, None, spectre, None,
                   date]
        return insdata, gpsdata

    def readRSI(self, filename, useUTCtime=None):
        """
        Reads a csv file exported from RSI's radassist. The file has a three
        line header.
        ...
        colnames = ['Lat', 'Long', 'Alt[m]', 'UtcTime', 'Laser_Alt [M]',
                    'RAD_ALT [M]', 'Pres', 'Temp', 'LineNum', 'UtcDate']
        # Detector specific data:
        perdetector = ['DetCount', 'LiveTime', 'Doserate']
        # ,'PPT_PRES','PPT_TEMP']
        ...
        with open(self.filename, "r", encoding='latin-1') as f:
            for idx, line in enumerate(f):
                ...
                if (header):
                    ...
                    if idx == 2:
                        ...
                        for ridx, roi in enumerate(roiidxs):
                            ...
                            while data[col] != '':
                                ...
                                col += 1
                        # Pressure and temperature may be stored as per
                        # detector data
                        if fields['Pres'] is None and 'PPT_PRES [mbar]' in data:  # noqa e501
                            pass
                    ...
                    # The RSI file may contain more spectra, the are ignored
                    # for the time being
                    # TODO: Look into reading more spectra from RSI-file
                    # Possible solution: Make a file with data from one
                    # detector, mark detector number per line, filter on
                    # detector number. On the other hand,
                    # this will make a mess if the data are not correctly
                    # filtered.
                    # Another solution: make separate data sets for the
                    # different detectors
                    ...
                    try:
                        ...
                        if useUTCtime:
                            timestamp = datetime.utcfromtimestamp(epoch).isoformat()  # noqa e501
                        else:
                            timestamp = datetime.fromtimestamp( epoch ).isoformat()  # noqa e501
                        # Timestamp may either be epoch is secounds
                        # or hh:mnm:ss ...
                    except ValueError:
                        # The utctime is not a number. Probably h:m:s
                        if not timestampwarned:
                            message = "Timestamp not as epoch time, assuming today UTC"  # noqa e501
                            level = Qgis.Warning
                            self.iface.messageBar().pushMessage("Data Loader",
                                                                message,
                                                                level=level)
                            timestampwarned = True
                        (h, m, s) = data[fields['UtcTime']].split(':')
                        epoch = int(datetime.combine(date.today(),
                                                     time(int(h), int(m),
                                                          int(s))).timestamp())
                        timestamp = data[fields['UtcTime']]
                        data[fields['UtcTime']] = epoch
                        if not fields['UtcDate'] is None:
                            timestamp = f"{data[fields['UtcDate']]}T{timestamp}" # noqa e501
                    # position data need some special treatment and shall not
                    # end up in fields
                    insdata = []
                    for field in fields:
                    ...
                    #       Write a wrapper to insertpoint that creates the
                    #       list from the dict
                    try:
                        ...
                        if self.abortOnFailure:
                            message = f"Error when importing {self.filename}" + \
                               "- Aborting, rerun and see python console for details"  # noqa e501
                            level = Qgis.Critical
                            self.iface.messageBar().pushMessage("Data Loader",
                                                                message,
                                                                level=level)
    ...
    def insertpoint(self, lat, lon, insdata, filename=None):
        ...
        Filename  be set . Makes most sense for data types where each spectrum
        ...
        if self.noimport00 and lat == 0.0 and lon == 0.0:
            message = "Problem when reading point, 0,0 latitude and longitude"
            level = Qgis.Warning
            self.iface.messageBar().pushMessage("Data Loader", message,
                                                level=level)
            print(message)
        ...
        for i in range(0, len(insdata)):
            ...
            # TODO: Make a custom exception so it is possible to fail out with
            #       more information
    ...
    def createlayer(self):
        ...
        # TODO: Show a reminder if project == EPSG3246,
        #       that this may not be what one wants
